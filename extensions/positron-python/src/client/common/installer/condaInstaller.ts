...
export class CondaInstaller extends ModuleInstaller {
    ...
    protected async getExecutionInfo(
        ...
        if ([Product.tensorboard, Product.ipykernel].map(translateProductToModule).includes(moduleName)) {
        ...
        if (flags & ModuleInstallFlags.reInstall || [Product.ipykernel].map(translateProductToModule).includes(moduleName)) {
